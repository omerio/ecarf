 ...
 */
package io.ecarf.core.cloud.task.impl.reason.phase2;
...
public class DoReasonTask6 extends CommonTask {
	 ...
	 * @param cloud
	 */
	public DoReasonTask6(VMMetaData metadata, CloudService cloud) {
		super(metadata, cloud);
		//this.executor = Utils.createFixedThreadPool(null);
	}
		...
		do {
			...
			for(QueryResult queryResult: queryResults) {
				try {
					...
					QueryStats stats = this.cloud.saveBigQueryResultsToFile(queryResult.getJobId(), queryResult.getFilename());
			...
			if(interimInferredTriples > 0) {
				...
				if(interimInferredTriples <= streamingThreshold) {
					...
					StorageObject file = this.cloud.uploadFileToCloudStorage(inferredTriplesFile, bucket);
					log.info("File " + file + ", uploaded successfully. Now loading it into big data.");
					
					String jobId = this.cloud.loadCloudStorageFilesIntoBigData(Lists.newArrayList(file.getUri()), table, false);
